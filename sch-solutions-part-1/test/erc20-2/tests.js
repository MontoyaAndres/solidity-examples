...
describe("ERC20 Token Exercise 2", () => {
  let deployer;
  ...
  it("Deploy depository and load receipt tokens", async () => {
    ...
    // Load receipt tokens into objects under `this` (e.g this.rAave)
    this.rAave = await ethers.getContractAt(
      "rToken",
      await this.depository.receiptTokens(AAVE_ADDRESS)
    );
    this.rUni = await ethers.getContractAt(
      "rToken",
      await this.depository.receiptTokens(UNI_ADDRESS)
    );
    this.rWeth = await ethers.getContractAt(
      "rToken",
      await this.depository.receiptTokens(WETH_ADDRESS)
    );
  });

  it("Deposit tokens tests", async () => {
    // Deposit Tokens
    ...
    expect(await this.rAave.balanceOf(this.aaveHolder.address)).to.be.equal(
      this.aaveAmount
    );
  ...
  it("Withdraw tokens tests", async () => {
    ...
    expect(await this.aave.balanceOf(this.aaveHolder.address)).to.be.equal(
      this.initialAAVEBalance
    );
    expect(await this.uni.balanceOf(this.uniHolder.address)).to.be.equal(
      this.initialUNIBalance
    );
    expect(await this.weth.balanceOf(this.wethHolder.address)).to.be.equal(
