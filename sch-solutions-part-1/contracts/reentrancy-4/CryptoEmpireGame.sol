...
contract CryptoEmpireGame is IERC1155Receiver {
    ...
    function listForSale(uint256 _nftId, uint256 _price) external {
        require(
            cryptoEmpireToken.balanceOf(msg.sender, _nftId) > 0,
        ...
        cryptoEmpireToken.safeTransferFrom(
            msg.sender,
            address(this),
            _nftId,
            AMOUNT,
    ...
    function stake(uint256 _nftId) external {
        ...
        require(
            !stakedNfts[msg.sender][_nftId],
            "NFT with the same tokenID cannot be staked again"
        );
